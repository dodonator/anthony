...
class Day:
    ...
    def to_dict(self) -> dict:
        """Returns Day as a dict."""
        day_dict = dict()
        day_dict["date"] = self.date
        entries = [(e.to_dict(), e_type) for (e, e_type) in self.entries]
        day_dict["entries"] = entries
        return day_dict

    @staticmethod
    def from_dict(day_dict: dict) -> Day:
        """Generates Day from dict."""
        date = day_dict.get("date")
        day = Day(date)

        entries = day_dict.get("entries")
        for element_dict, element_type in entries:
            ...
            day.entries.append(element, element_type)
        return day
